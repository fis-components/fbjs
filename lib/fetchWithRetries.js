    ...
    function sendTimedRequest() {
      ...
      var requestTimeout = setTimeout(function () {
        ...
        if (shouldRetry(requestsAttempted)) {
          "production" !== 'production' ? warning(false, 'fetchWithRetries: HTTP timeout, retrying.') : undefined;
          retryRequest();
        } else {
      ...
      request.then(function (response) {
        ...
        if (isRequestAlive) {
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            "production" !== 'production' ? "production" !== 'production' ? warning(false, 'fetchWithRetries: HTTP error, retrying.') : undefined : undefined, retryRequest();
          } else {
